import { toTeamSongModel } from '../song/mapper'
import type { Conti, ContiSong, SharedConti } from './model'
import type {
  ContiResponseDto,
...
import { normalizeWorshipTime } from './worship-time'
...
export const toContiSongModel = (dto: ContiSongResponseDto): ContiSong => {
  const mappedTeamSong = dto.teamSong
    ? toTeamSongModel(dto.teamSong)
    : undefined
...
export const toSharedContiModel = (
  dto: SharedContiResponseDto
): SharedConti => ({
  id: dto.id,
  title: dto.title,
  worshipDate: dto.worshipDate,
