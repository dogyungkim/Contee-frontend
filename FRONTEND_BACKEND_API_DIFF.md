...
프론트는 대시보드 전용 `DashboardContiDto`와 `DashboardConti` 타입으로 축약 응답을 처리한다.

백엔드 대시보드의 `songs`도 축약 응답인데, 프론트는 일반 `Song` 타입으로 선언해 `createdAt`, `updatedAt`을 필수로 기대한다.

적용한 수정:

- `recentContis`에 대시보드 전용 DTO와 모델을 적용했다.

### 3.4 송폼 파트의 nullable 필드 (수정 완료)

백엔드 송폼 응답 예시에서 `id`, `partOrder`는 `null`일 수 있다.

기존 프론트는 두 필드를 `number`로만 선언했다.
...
- 기본 송폼과 콘티 송폼의 응답 DTO 및 모델을 nullable 타입으로 변경했다.
- UI 송폼 ID 생성 시 `part.id ?? index`로 null을 안전하게 처리한다.

### 3.5 콘티 상태 제거 (수정 완료)

최신 콘티 응답에는 `status`, `publishedAt`이 없고 상태 변경 API도 제거되었다.

적용한 수정:

- 콘티 DTO와 모델에서 상태 필드를 제거했다.
- 상태 배지, 발행·보관 버튼, 상태 기반 편집 제한을 제거했다.
- `PATCH /api/v1/contis/{contiId}/status` 호출과 관련 목 구현을 제거했다.
...
6. ~~콘티 순서 변경 요청을 `{ contiSongIds }`로 통일한다.~~ (완료)
7. ~~대시보드 전용 DTO를 분리한다.~~ (완료)
8. ~~`/teams/{teamId}/contis`를 사용하도록 콘티 목록 조회를 변경한다.~~ (완료)
