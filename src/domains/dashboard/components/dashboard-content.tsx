...
export function DashboardContent() {
  ...
  const hasUpcomingService =
    summary.nextServiceLabel.trim().length > 0 ||
    summary.nextServiceDateLabel.trim().length > 0

  return (
    <>
      <DashboardHeader canCreateConti={canCreate} />
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div className="min-w-0">
                ...
                <div className="type-page-title mt-3">
                  팀의 최근 준비 흐름을
                  <br className="hidden sm:block" />한 화면에서 확인하세요.
                </div>
                <p className="type-page-description mt-3 max-w-xl">
                  콘티 작성, 곡 확인, 공유 상태를 하나의 작업 표면 안에서 관리할
                  수 있습니다.
                </p>
              </div>
            ...
            <CardContent className="grid gap-3">
              {recentContis.length === 0 ? (
                ...
                recentContis.map((conti) => (
                  <div
                    key={conti.id}
                    className="group grid gap-3 rounded-xl border p-4 transition-colors hover:bg-[#f3f3f3] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
                  >
                    <div className="min-w-0">
                      <div className="type-body-sm truncate font-medium">
                        {conti.title}
                      </div>
                      <div className="type-badge mt-1 text-muted-foreground">
                        {format(new Date(conti.worshipDate), 'yyyy. M. d', {
                          locale: ko,
                        })}{' '}
                        · {conti.songCount}곡
                      </div>
                  ...
                  filteredSongs.map((song) => (
                    <div key={song.id} className="rounded-xl border p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="type-body-sm min-w-0 break-words font-medium">
                          {song.title}
                        </div>
            ...
            <CardContent className="grid gap-2">
              {canCreate && (
                <Button asChild className="justify-start" variant="outline">
                  <Link href="/dashboard/contis/new">
                    <ListMusic className="mr-2 h-4 w-4" />새 콘티 작성
                  </Link>
                </Button>
              )}
              <Button asChild className="justify-start" variant="outline">
              ...
              <Button asChild className="justify-start" variant="outline">
                <Link href="/dashboard/songs">
                  <Search className="mr-2 h-4 w-4" />곡 라이브러리
                </Link>
              </Button>
            </CardContent>
          </Card>
              ...
              {activities.length === 0 ? (
                ...
                activities.map((a) => (
                  <div key={a.id} className="rounded-xl border p-4">
                    <div className="type-badge text-muted-foreground">
                      {a.timeLabel}
                    </div>
