...
export const useContis = (
  teamId: string | null,
  params: ContiSearchParamsDto = { page: 0, size: 100 }
) => {
  return useQuery({
...
export const useUpdateConti = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: contiKeys.teamLists(data.teamId),
      })
    },
  })
}
...
export const useEnableExternalShare = () => {
  ...
  return useMutation({
    ...
    onSuccess: (externalShare, contiId) => {
      const previous = queryClient.getQueryData<Conti>(
        contiKeys.detail(contiId)
      )
      ...
      if (previous) {
        ...
        queryClient.invalidateQueries({
          queryKey: contiKeys.teamLists(previous.teamId),
        })
      } else {
        queryClient.invalidateQueries({ queryKey: contiKeys.all })
      }
