...
export async function getTeamSongs(
  teamId: string,
...
export async function updateTeamSongFavorite(
  teamId: string,
  teamSongId: string,
  isFavorite: boolean
): Promise<TeamSong> {
  const { data } = await (isFavorite
    ? apiClient.put<ApiResponse<TeamSongResponseDto>>(
        `/api/v1/teams/${teamId}/songs/${teamSongId}/favorite`
      )
    : apiClient.delete<ApiResponse<TeamSongResponseDto>>(
        `/api/v1/teams/${teamId}/songs/${teamSongId}/favorite`
      ))
  return toTeamSongModel(data.data)
}

export async function deleteTeamSong(
  teamId: string,
  teamSongId: string
): Promise<void> {
