...
} from '@/domains/song/components/song-editor-dialog'
...
import {
  filterSongLibrary,
  findDuplicateSong,
} from '@/domains/song/utils/song-library'
...
export default function SongsPage() {
  ...
  const { data: songs = [], isLoading: isSongsLoading } =
    useTeamSongs(selectedTeamId)
  ...
  const { mutateAsync: updateFavorite } = useUpdateTeamSongFavorite()
  const { mutateAsync: updateSongForm, isPending: isUpdatingForm } =
    useUpdateSongForm()
  ...
  const selectedSongForm = useMemo(
    () => mapApiSongFormToUi(songForm?.parts ?? []),
    [songForm]
  )
  ...
  const handleSave = async (request: CreateTeamSongRequest) => {
    ...
    if (duplicate) {
      toast.error(
        `이미 등록된 곡입니다: ${duplicate.title}${duplicate.artist ? ` · ${duplicate.artist}` : ''}`
      )
      return
    }
    ...
    try {
      ...
      toast.error(
        getContiApiErrorMessage(error, '곡 정보를 저장하지 못했습니다.')
      )
    }
  }
  ...
  const handleToggleFavorite = async (song: TeamSong) => {
    ...
    try {
      ...
      toast.error(
        getContiApiErrorMessage(error, '즐겨찾기를 변경하지 못했습니다.')
      )
    }
  }
